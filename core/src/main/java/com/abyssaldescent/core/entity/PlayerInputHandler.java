package com.abyssaldescent.core.entity;

import com.abyssaldescent.core.command.AttackCommand;
import com.abyssaldescent.core.command.DashCommand;
import com.abyssaldescent.core.command.MoveCommand;
import com.badlogic.gdx.Gdx;
import com.badlogic.gdx.Input;
import com.badlogic.gdx.InputAdapter;

/**
 * Translates libGDX keyboard/mouse input into {@link com.abyssaldescent.core.command.Command}
 * objects pushed to the {@link Player}.
 *
 * <ul>
 *   <li><b>WASD</b> — movement</li>
 *   <li><b>Left Mouse Button</b> — melee attack</li>
 *   <li><b>Space / Right Mouse Button</b> — dash (dodge-roll)</li>
 * </ul>
 *
 * Call {@link #update()} once per frame <em>before</em> {@link Player#update(float)}.
 */
public final class PlayerInputHandler extends InputAdapter {

    private final Player player;

    private boolean attackPressed;
    private boolean dashPressed;

    public PlayerInputHandler(Player player) {
        this.player = player;
    }

    /**
     * Polls keyboard state and pushes the appropriate commands.
     * Should be called each frame from the game loop.
     */
    public void update() {
        // ── Movement (WASD) ─────────────────────────────────────────────────
        float ix = 0;
        float iy = 0;
        if (Gdx.input.isKeyPressed(Input.Keys.W)) iy += 1;
        if (Gdx.input.isKeyPressed(Input.Keys.S)) iy -= 1;
        if (Gdx.input.isKeyPressed(Input.Keys.A)) ix -= 1;
        if (Gdx.input.isKeyPressed(Input.Keys.D)) ix += 1;

        player.pushCommand(new MoveCommand(ix, iy));

        // ── One-shot actions (buffered via touchDown / keyDown) ─────────────
        if (attackPressed) {
            player.pushCommand(AttackCommand.INSTANCE);
            attackPressed = false;
        }
        if (dashPressed) {
            player.pushCommand(DashCommand.INSTANCE);
            dashPressed = false;
        }
    }

    // ── InputProcessor callbacks ────────────────────────────────────────────

    @Override
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        if (button == Input.Buttons.LEFT) {
            attackPressed = true;
            return true;
        }
        if (button == Input.Buttons.RIGHT) {
            dashPressed = true;
            return true;
        }
        return false;
    }

    @Override
    public boolean keyDown(int keycode) {
        if (keycode == Input.Keys.SPACE) {
            dashPressed = true;
            return true;
        }
        return false;
    }
}
